...
class DQN():
    def __init__(self, state_dim, action_dim, n_latent_var, lr, betas, gamma, model, device, batch=32):
    ...
    def update(self, memory):
        if len(memory) < self.batch:
            return
        batch, idx, weight = memory.sample(self.batch)
